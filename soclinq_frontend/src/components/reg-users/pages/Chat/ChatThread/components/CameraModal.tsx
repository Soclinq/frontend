...
import styles from "./styles/ChatCameraModal.module.css";
...
type Props = {
  onClose: () => void;
  onSend: (payload: { files: File[]; caption: string }) => Promise<void> | void;
};
...
export default function CameraModal({ onClose, onSend }: Props) {
  ...
  async function confirmSend() {
    ...
    try {
      await onSend({ files, caption }); // ✅ await
      closeNow();
    } catch (err) {
      notify({
        type: "error",
